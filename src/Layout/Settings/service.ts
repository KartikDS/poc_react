...
export async function getSettingsDetail(action: ACTION, token: string): Promise<unknown> {
  const { payload } = action as { payload: KEYPAIR };
  const res: ReturnType<any> = await fetch(
    `${process.env.BACKEND}/v1/api/administrator/get-settings/${payload?.model}`,
    {
      method: 'GET',
      cache: 'no-store',
      headers: {
...
export async function getGlobalBranding(): Promise<unknown> {
  const res: ReturnType<any> = await api(`${process.env.BACKEND}/v1/api/administrator/get-settings/branding`, 'GET');
  return res;
}

//getDetailByid

export async function getDetailByid(action: ACTION, token: string): Promise<unknown> {
  const { payload } = action as { payload: KEYPAIR };
  const res: ReturnType<any> = await fetch(`${process.env.BACKEND}/v1/api/administrator/get-details/${payload?.id}`, {
    method: 'GET',
    headers: {
