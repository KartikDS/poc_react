...
export const formatDate = (date: string | Date, type?: string | null | undefined, iso?: boolean) => {
  ...
  const formattedDate = moment(date)
    .tz('Asia/Kolkata')
    .format(type || 'MM/DD/YYYY hh:mm:ss A'); // Format date as MM/DD/YYYY hh:mm:ss A with timezone offset +10:00 (Australia/Sydney)
...
export const formatDateOnly = (date: string | Date, type?: string | null | undefined, iso?: boolean) => {
  ...
  // Format date as MM/DD/YYYY if no type is provided
  const formattedDate = moment(date)
    .tz('Asia/Kolkata')
    .format(type || 'MM/DD/YYYY'); // Changed to return only date
...
function conValToString(value: any) {
  if (typeof value === 'boolean') {
    return value ? 1 : 0;
  }
  return value;
}
export function jsonToFormData(json: any, formData = new FormData(), parentKey: string | null = null) {
      ...
      if (typeof value === 'object' && !Array.isArray(value)) {
        ...
        value.forEach((item, index) => {
          if (typeof item !== 'object') {
            formData.append(`${fullKey}[${index}]`, conValToString(item));
          } else {
            jsonToFormData(item, formData, `${fullKey}[${index}]`);
          }
        });
      } else {
        formData.append(fullKey, conValToString(value));
      }
    }
  }
